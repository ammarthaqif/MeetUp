          ...
          <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 sm:p-10 shadow-2xl shadow-black/80 space-y-7">
            ...
            {activeTab === 'token' && (
              ...
              >
                ...
                {/* Status alerts */}
                {errorMsg && (
                  <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-300 text-xs space-y-2 animate-fadeIn">
                    ...
                    {/* Quick instant recovery shortcuts */}
                    <div className="pt-1.5 border-t border-rose-500/20 flex flex-wrap items-center gap-1.5">
                      <span className="text-[10px] text-slate-400">Quick Unlock:</span>
                      <button
                        type="button"
                        onClick={() => {
                          resetRateLimit();
                          setErrorMsg(null);
                          setTokenInput('ACME-ADMIN-2026');
                          handleVerifyToken('ACME-ADMIN-2026');
                        }}
                        className="px-2 py-1 rounded bg-indigo-600/40 hover:bg-indigo-600 text-white font-mono text-[10px] font-bold transition-all cursor-pointer"
                      >
                        ACME-ADMIN-2026
                      </button>
                      <button
                        ...
                        onClick={() => {
                          resetRateLimit();
                          setErrorMsg(null);
                          setTokenInput('ACME-STAFF-101');
                          handleVerifyToken('ACME-STAFF-101');
                        }}
                        className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-200 font-mono text-[10px] transition-all cursor-pointer"
                      >
                        ACME-STAFF-101
                      </button>
                      <button
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {tenants.slice(0, 4).map(t => {
                        const adminKey = accessKeys.find(k => k.tenantId === t.id && k.active && k.role === 'company_admin')?.token || `${t.code}-ADMIN-2026`;
                        const staffKey = accessKeys.find(k => k.tenantId === t.id && k.active && (k.role === 'staff' || !k.role))?.token || `${t.code}-STAFF-101`;

                        return (
                          <div
                            key={t.id}
                            className="p-2.5 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5"
                          >
                            <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1.5">
                              <button
                                type="button"
                                onClick={() => {
                                  setTokenInput(adminKey);
                                  handleVerifyToken(adminKey);
                                }}
                                title="Login as Company Admin"
                                className="flex-1 px-2 py-1 rounded bg-purple-950/60 hover:bg-purple-900 border border-purple-800/60 text-purple-200 font-mono text-[10px] text-center font-bold transition-all cursor-pointer truncate"
                              >
                                {adminKey}
                              </button>
                              <button
                                type="button"
                                onClick={() => {
                                  setTokenInput(staffKey);
                                  handleVerifyToken(staffKey);
                                }}
                                title="Login as Staff"
                                className="flex-1 px-2 py-1 rounded bg-slate-900 hover:bg-slate-800 border border-slate-700/80 text-slate-300 font-mono text-[10px] text-center transition-all cursor-pointer truncate"
                              >
                                {staffKey}
                              </button>
                            </div>
